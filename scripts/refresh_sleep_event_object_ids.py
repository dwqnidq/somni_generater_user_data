"""文件作用：仅对 language 为 zh 的 sleep event 刷新 _id，并修正「AI主动干预」的 related_event_id。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "遍历 sleep_events：仅处理 language=zh 的记录——写入新的 _id；若 event_type 为「AI主动干预」，"
            "则将 related_event_id 设为列表中上一条（索引减一）的 _id；其它 language 不修改。"
        )
    )
    parser.add_argument(
        "--input",
