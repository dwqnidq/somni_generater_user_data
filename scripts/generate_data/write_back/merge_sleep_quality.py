#!/usr/bin/env python3
"""sleep_quality 的 quality_analysis_module[0] append 到 sleep_report.quality_analysis.module。"""
...
def merge_sleep_quality_to_report(
    uid: str,
    output_dir: str,
    ...
    q_path = os.path.join(output_dir, f"{uid}_sleep_quality.json")
    rep_path = os.path.join(output_dir, f"{uid}_sleep_report.json")
    q_rows = load_json_list(q_path)
    ...
    for qr in q_rows:
        if not isinstance(qr, dict):
            continue
        rd = str(qr.get("record_date") or "")
        if not rd or not record_date_in_range(rd, start_date, end_date):
            continue
        mod = qr.get("quality_analysis_module")
...
def main() -> None:
    ...
    args = p.parse_args()
    ok, miss = merge_sleep_quality_to_report(
        args.uid.strip(),
        args.output_dir,
        start_date=args.start_date.strip() or None,
        end_date=args.end_date.strip() or None,
    )
    print(json.dumps({"sleep_quality_appended": ok, "sleep_quality_skipped": miss}, ensure_ascii=False))
