#!/usr/bin/env python3
"""sleep_auditory 的 auditory_module 写回 sleep_report.quality_analysis.auditory.module。"""
...
def merge_sleep_auditory_to_report(
    uid: str,
    output_dir: str,
    ...
    a_path = os.path.join(output_dir, f"{uid}_sleep_auditory.json")
    rep_path = os.path.join(output_dir, f"{uid}_sleep_report.json")
    a_rows = load_json_list(a_path)
...
def main() -> None:
    ...
    args = p.parse_args()
    ok, miss = merge_sleep_auditory_to_report(
        args.uid.strip(),
        args.output_dir,
        start_date=args.start_date.strip() or None,
        end_date=args.end_date.strip() or None,
    )
    print(json.dumps({"sleep_auditory_updated": ok, "sleep_auditory_skipped": miss}, ensure_ascii=False))
