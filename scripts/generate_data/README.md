...
| `generate_sleep_map_pool.py` | **推荐**：按**指定日期范围**一键生成 `somni_sleep_analysis.json` + `somni_sleep_district.json`（multi_user → aggregated **合并八人格** → adjust_ranking 保证人格每日前 10）。须已有人格 `{uid}_somni_sleep_analysis.json`。 | `python scripts/generate_data/generate_sleep_map_pool.py --start-date 2026-05-01 --end-date 2026-05-31`；可选：`--aggregated-only`、`--no-merge-personas`、`--skip-ranking-adjust`。 |
