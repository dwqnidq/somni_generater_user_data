#!/usr/bin/env python3
"""
将睡眠干预方案 JSON 写入 MongoDB 集合 somni_temp_plans。

- 连接串：读取项目根目录 .env 中的 MONGODB_URI（或 MONGO_URI），不设代码内默认密钥。
- 默认数据文件：output2/sleep_intervention_schemes.json（与 generate_sleep_intervention_schemes_from_aaa_md 一致）。
- 按 mhr_codes 整组 upsert，重复执行可更新同一人格方案。
- 插入前将 create_time、update_time 从字符串等转为 BSON Date（UTC naive datetime）。

用法（在项目根目录）:
  python scripts/insert_data/insert_sleep_intervention_schemes.py
  python scripts/insert_data/insert_sleep_intervention_schemes.py --file output2/sleep_intervention_schemes.json
...
DEFAULT_JSON = PROJECT_ROOT / "output" / "sleep_intervention_schemes.json"
...
def _to_mongo_date(value: object) -> datetime | None:
    """
    转为 PyMongo 可编码的 BSON Date：UTC、无时区信息的 datetime（与 insert_somni_records 中日期字段一致）。
    """
    if value is None:
        return None
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="插入睡眠干预方案到 MongoDB")
    ap.add_argument(
        "--file",
        type=Path,
