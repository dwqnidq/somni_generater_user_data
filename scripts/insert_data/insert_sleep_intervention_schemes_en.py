#!/usr/bin/env python3
"""
将英文睡眠干预方案 JSON 写入 MongoDB 集合 somni_temp_plans。

- 默认数据文件：output/sleep_intervention_schemes_en.json（由 translate_sleep_intervention_schemes.py 生成）。
- 连接：项目根 .env 中 MONGODB_URI（或 MONGO_URI）；不设代码内默认密钥。
- 按 mhr_codes + language + type（若有）upsert，避免覆盖中文方案。
...
DEFAULT_JSON = PROJECT_ROOT / "output" / "sleep_intervention_schemes_en.json"
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="插入英文睡眠干预方案（init）到 MongoDB")
    ap.add_argument(
        "--file",
        type=Path,
