#!/usr/bin/env python3
"""
将「干预专用」睡眠方案 JSON 写入 MongoDB 集合 somni_temp_plans。

- 默认数据文件：output/sleep_intervention_schemes_interv.json（由 build_sleep_intervention_schemes_interv.py 生成）。
- 连接：项目根 .env 中 MONGODB_URI（或 MONGO_URI）；不设代码内默认密钥（与 insert_sleep_intervention_schemes.py 一致）。
- 按 mhr_codes 整组 upsert（与 init 版脚本相同过滤键，执行本脚本会用 interv 文档覆盖同 mhr_codes 的旧文档）。
...
DEFAULT_JSON = PROJECT_ROOT / "output" / "sleep_intervention_schemes_interv.json"
...
def _to_mongo_date(value: object) -> datetime | None:
    """转为 BSON Date：UTC、无时区信息的 datetime。"""
    if value is None:
        return None
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="插入干预版睡眠方案（interv）到 MongoDB")
    ap.add_argument(
        "--file",
        type=Path,
